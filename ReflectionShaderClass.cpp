...
bool ReflectionShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
	...
	error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/reflectionvs.hlsl");
	if (error != 0) {
		return false;
	}

	error = wcscpy_s(psFilename, 128, L"../DirectXEngine/reflectionps.hlsl");
	if (error != 0) {
		return false;
	}
...
bool ReflectionShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	result = D3DCompileFromFile(vsFilename, nullptr, nullptr, "ReflectionVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	result = D3DCompileFromFile(psFilename, nullptr, nullptr, "ReflectionPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
