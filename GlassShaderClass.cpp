...
bool GlassShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
    ...
    int error;
    bool result;
    error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/glassvs.hlsl");
    if (error != 0) {
        return false;
    }

    error = wcscpy_s(psFilename, 128, L"../DirectXEngine/glassps.hlsl");
    if (error != 0) {
        return false;
    }
...
bool GlassShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
    ...
    result = D3DCompileFromFile(vsFilename, nullptr, nullptr, "GlassVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
    ...
    result = D3DCompileFromFile(psFilename, nullptr, nullptr, "GlassPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
    ...
    result = device->CreateBuffer(&glassBufferDesc, nullptr, &m_glassBuffer);
    if (FAILED(result)) {
        return false;
    }
...
bool GlassShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext,
    ...
    dataPtr2 = static_cast <GlassBufferType*> (mappedResource.pData);
