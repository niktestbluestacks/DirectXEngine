...
bool MultiTextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
    ...
    error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/multitexturevs.hlsl");
    if (error != 0) {
        return false;
    }

    error = wcscpy_s(psFilename, 128, L"../DirectXEngine/multitextureps.hlsl");
    if (error != 0) {
        return false;
    }
...
bool MultiTextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
    ...
    result = D3DCompileFromFile(vsFilename, nullptr, nullptr, "MultiTextureVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
