...
bool NormalMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    int error;
    error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/normalmapvs.hlsl");
    if (error != 0) {
        return false;
    }

    error = wcscpy_s(psFilename, 128, L"../DirectXEngine/normalmapps.hlsl");
    if (error != 0) {
        return false;
    }
...
bool NormalMapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
    ...
    result = D3DCompileFromFile(vsFilename, nullptr, nullptr, "NormalMapVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
    ...
    result = D3DCompileFromFile(psFilename, nullptr, nullptr, "NormalMapPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
