...
bool LightMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
    ...
    int error;
    error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/lightmapvs.hlsl");
    if (error != 0) {
        return false;
    }

    error = wcscpy_s(psFilename, 128, L"../DirectXEngine/lightmapps.hlsl");
    if (error != 0) {
        return false;
    }
...
bool LightMapShaderClass::InitializeShader(ID3D11Device* device,
    ...
    result = D3DCompileFromFile(vsFilename, nullptr, nullptr, "LightMapVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
...
bool LightMapShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
    ...
    deviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_matrixBuffer);    // Set shader texture resources in the pixel shader.
