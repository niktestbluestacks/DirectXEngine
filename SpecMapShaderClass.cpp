...
bool SpecMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
    ...
    int error;
    error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/specmapvs.hlsl");
    if (error != 0) {
        return false;
    }

    error = wcscpy_s(psFilename, 128, L"../DirectXEngine/specmapps.hlsl");
    if (error != 0) {
        return false;
    }
...
bool SpecMapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
    ...
    result = D3DCompileFromFile(vsFilename, nullptr, nullptr, "SpecMapVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
